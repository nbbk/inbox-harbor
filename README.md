...
- 🌐 **多邮局统一集成管理**
  ...
  - 批量监控账号活跃状态，一键在线健康检测。
- ⚡ **克制的增量取件**
  - Gmail 使用官方 `historyId`，Microsoft 使用 Graph `deltaLink`，只获取自上次同步以来的变化。
  - 首次建立增量游标或游标失效重建时，历史邮件仅归档、不当作新邮件推送；同步账户之间并发，单账户内部防止重入。
- 🔑 **智能验证码与链接剥离**
...
- 邮件接口支持分页与高级筛选：关键词、发件人、收件人、账户、分类、日期、验证码和收发方向；页面提供上一页、下一页。
- 邮件详情支持手动纠正分类、收藏、置顶、已读状态，以及收件人、抄送和附件元信息展示；可建立发件人或域名分类规则。
- 每个邮箱独立显示同步状态、最后同步时间与错误，并支持暂停同步和单独取件；后台采用至少 30 秒的账户级调度与失败退避，避免高频请求触发服务商限流。
- 当前发送正文为纯文本。Google 通过 Gmail API 发送，Microsoft 通过 Microsoft Graph 发送；上游拒绝时页面会显示失败，不会伪报成功。

## 通知渠道配置

所有通知渠道只发送发件人、账户、时间、标题、摘要和签名查看链接，不直接塞入冗长正文。查看链接默认 30 天有效，可在通知设置中配置为 1–365 天；链接免登录且只读。通知投递按“邮件 × 渠道”去重，失败最多自动重试 3 次。所有密钥字段均为只写：保存后界面只显示“已配置”，不会回显原值。
